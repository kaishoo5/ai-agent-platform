...
public class ConversationSummaryService {
    ...
    private String createSystemPrompt() {
        ...
        동일한 목적의 web_search를 불필요하게 반복하지 마세요.
        이미 얻은 검색 결과만으로 사용자의 질문에 충분히 답변할 수 있다면
        추가 web_search를 호출하지 말고 해당 결과를 사용하여 최종 답변을 작성하세요.

        추가 검색은 기존 검색 결과만으로 답변하기 어렵거나
        서로 다른 정보가 추가로 필요한 경우에만 수행하세요.

        사용자가 사용한 언어와 동일한 언어로 최종 답변을 작성하세요.
        사용자가 한국어로 질문하면 반드시 한국어로 답변하세요.

        web_search 등 Tool의 결과가 영어 또는 다른 언어이더라도
        Tool 결과의 언어를 따라가지 말고 사용자의 질문 언어를 유지하세요.

        고유명사, 제품명, 기술명 등은 필요한 경우 원문 표기를 유지할 수 있습니다.
